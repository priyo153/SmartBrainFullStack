...
class App extends Component {
  ...
  calculateFaceLocation = data => {
    ...
    for (let item of data.outputs[0].data.regions) {
      try {
        ...
        clarifyface = item.regionInfo.boundingBox;

        coords.push({
          leftcol: width * clarifyface.leftCol,
          toprow: height * clarifyface.topRow,
          rightcol: width - clarifyface.rightCol * width,
          bottomrow: height - clarifyface.bottomRow * height
        });
      }
    }
  ...
  render() {
    ...
    if (this.state.route === "signin") {
      ...
      x = <Register onRouteChange={this.onRouteChange} ApiUrl={ApiUrl} />;
    ...
    return (
      <div className="App">
        ...
        <div>
          <h3 className="ttu blue uppercase pt5">
            This project was built by Priyojit Chatterjee
          </h3>
        </div>
      </div>
    );
